"""
TrustCatalog dashboard.

Run:  streamlit run dashboard/app.py

The dashboard is a pure READER. Every number on screen comes from the SQLite
database written by scripts/run_pipeline.py - nothing is hard-coded and no
model is retrained here (except inside the stress test, which only *scores*).
"""
...
import config
from src import anomaly_detection, auth, database, evaluation, explanations as explain_mod
...
def role_gateway():
    """Render the role-selection and login/registration gateway."""
    st.markdown("<div class='tc-title'>TrustCatalog</div>", unsafe_allow_html=True)
    st.markdown("<div class='tc-sub'>Secure marketplace access with role-based accounts</div>", unsafe_allow_html=True)
    st.write("")
    ...
    if st.session_state.selected_role is None:
        ...
        if c1.button("🛒 Customer", use_container_width=True, type="primary"):
            st.session_state.selected_role = "CUSTOMER"
            st.rerun()
        if c2.button("🏪 Seller", use_container_width=True, type="primary"):
            st.session_state.selected_role = "SELLER"
            st.rerun()
        if c3.button("🔐 Admin", use_container_width=True):
            st.session_state.selected_role = "ADMIN"
            st.rerun()
        st.info("Select your role to continue. You will choose registration on your first visit and login on later visits.")
        return False

    role = st.session_state.selected_role
    st.button("← Change role", on_click=lambda: st.session_state.update(selected_role=None), key="change_role")
    title = {"CUSTOMER": "Customer Access", "SELLER": "Seller Access", "ADMIN": "Admin Access"}[role]
    st.subheader(title)

    if role in {"CUSTOMER", "SELLER"}:
        tab_login, tab_register = st.tabs(["Login", "First-time Registration"])
        with tab_login:
            ...
            if submitted:
                ...
                if user:
                    ...
                    st.error("Incorrect credentials or selected role.")
        with tab_register:
            with st.form(f"register_{role}"):
                full_name = st.text_input("Full Name")
                username = st.text_input("Username")
                email = st.text_input("Email")
                password = st.text_input("Password", type="password")
                confirm = st.text_input("Confirm Password", type="password")
                store_name = st.text_input("Store Name") if role == "SELLER" else ""
                submitted = st.form_submit_button("Create Account", type="primary", use_container_width=True)
            if submitted:
                if password != confirm:
                    st.error("Passwords do not match.")
                else:
                    ok, message = auth.register_user(full_name, username, email, password, role, store_name)
                    (st.success if ok else st.error)(message)
    else:
        st.caption("Admin accounts are created privately; public visitors cannot register as administrators.")
        with st.form("admin_login"):
            identifier = st.text_input("Admin Username or Email")
            password = st.text_input("Admin Password", type="password")
            submitted = st.form_submit_button("Secure Admin Login", type="primary", use_container_width=True)
        if submitted:
...
def customer_home(user):
    st.title("🛒 Customer Home")
    st.success(f"Welcome, {user['full_name']}!")
    st.info("Customer marketplace features can be connected here without exposing TrustCatalog's internal risk controls.")
    st.metric("Account type", "Customer")


def seller_home(user):
    st.title("🏪 Seller Dashboard")
    st.success(f"Welcome, {user['full_name']}!")
    st.info("Seller product, inventory and order features can be connected here while keeping the TrustCatalog risk engine private.")
    st.metric("Account type", "Seller")
...
def main():
    ...
    if user["role"] == "CUSTOMER":
        with st.sidebar:
            st.caption(f"Signed in as {user['full_name']} (Customer)")
    ...
    if user["role"] == "SELLER":
        with st.sidebar:
            st.caption(f"Signed in as {user['full_name']} (Seller)")
            if st.button("Log out", use_container_width=True):
                st.session_state.auth_user = None
                st.session_state.selected_role = None
                st.rerun()
    ...
    st.markdown(f"<div class='tc-title'>{config.PROJECT_NAME}</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='tc-sub'>{config.PROJECT_SUBTITLE}</div>", unsafe_allow_html=True)
    st.write("")
    ...
    with st.sidebar:
        st.markdown(f"### {config.PROJECT_NAME}")
        st.caption(f"Signed in as {user['full_name']} (Admin)")
        if st.button("Log out", use_container_width=True):
            st.session_state.auth_user = None
            st.session_state.selected_role = None
            st.rerun()
        ...
        if source == "DEMO":
            st.warning("Running on **DEMO DATA** (synthetic).")
        else:
            st.success("Running on the **Olist** dataset.")
        st.caption(f"Last pipeline run: {meta.get('run_at', 'unknown')}")
        st.caption(f"Feature/outcome cutoff: {str(meta.get('cutoff_date', ''))[:10]}")
